...
async function convertContentUrlToIssueUrl (url) {
...
async function createIssueUrl (repoUrl, branch) {
  ...
  if (parsedBranch != null && parsedBranch.length === 2) {
    return repoUrl + '/issues/' + parsedBranch[1]
  }
  return null
}
      ...
      const issueUrl = context.payload.issue
        ...
        : context.payload.pull_request
          ...
          : context.payload.project_card && context.payload.project_card.content_url !== undefined
            ? await convertContentUrlToIssueUrl(context.payload.project_card.content_url)
            : context.payload.ref_type !== undefined && context.payload.ref_type === 'branch'
              ? await createIssueUrl(context.payload.repository.html_url, context.payload.ref)
              : null
      logger.trace(`Event received for ${webhookName}`)

      // Only continue if issueUrl is not null.
      if (issueUrl != null) {
